...
$query = mysqli_query($conectar, "SELECT * FROM empresa WHERE id_empresa = '".$id_empresa."'");
		...
		while($linha = mysqli_fetch_array($query)) {
			$igreja = $linha['nome_empresa'];  $cnpj_igreja = $linha['cnpj_cpf_empresa']; $endereco_igreja = $linha['endereco'];
			$complemento_igreja = 	$linha['complemento']; $bairro_igreja = $linha['bairro'];$cidade_igreja = $linha['cidade'];
			$cep_igreja = $linha['cep']; $uf_igreja = $linha['uf'];
...
?>
	...
	<div class="form-group">
		...
		$data = date('d/m/Y');
				...
				<p><strong>Cláusula Quinta:</strong> Para a execução dos serviços constantes da Cláusula Primeira a CONTRATANTE pagará à CONTRATADA os honorários profissionais no valor de R$  via depósito ou cobrança bancária. O valor não inclui as taxas de registro em Cartório e de honorários advocatícios.</p>
