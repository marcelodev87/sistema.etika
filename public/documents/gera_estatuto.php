...
$query = mysqli_query($conectar, "SELECT * FROM empresa WHERE id_empresa = '".$id_empresa."'");

		while($igreja = mysqli_fetch_array($query)) {
				$nome_igreja = $igreja['nome_empresa']; $endereco_igreja = $igreja['endereco']; $complemento_igreja = $igreja['complemento']; $bairro_igreja = $igreja['bairro']; $cidade_igreja = $igreja['cidade']; $uf_igreja = $igreja['uf'];
					$cep_igreja = $igreja['cep'];

				$igrejaM = strtoupper($nome_igreja);
		}

// BUSCA PRESIDENTE ---------------------------- BUSCA PRESIDENTE ------------------------------------ BUSCA PRESIDENTE
				...
				while($linha_presidente = $result_presidente->fetch_assoc()) {
					...
					$nome_presidente = $linha_presidente['nome'];
				}
				}
// MEMBROS FUNDADORES ------------------------------------ MEMBROS FUNDADORES ----------------------------------MEMBROS FUNDADORES
//$fundadores = ( isset($_POST['fundadores']) )  ? true : null;

if (isset($_POST['fundadores'])) {
	$fundadores ="<span style='color:blue !important;'>tem como fundadores o ".$_POST['fundadores']."</span>";
}else{
	$fundadores ="";
}

// SEDE ----------------------------------------------------- SEDE ---------------------------------------- SEDE
$sede = "<span style='color:blue !important;'>".$_POST['sede']."</span>";
...
switch($_POST['vice']){
		...
		case "1":
			// INDICA FORMAÇÃO DA DIRETORIA
			$diretoria_vice = "<span style='color:red !important;'>Vice-Presidente</span>";
			break;

		case "0":
			//$assume_presidencia = "<span style='color:red !important;'>o Tesoureiro ocupará o cargo em caso de vacância;</span>";
			$vice = "";
			// INDICA FORMAÇÃO DA DIRETORIA
			$diretoria_vice = "";
			break;
...
// VERIFICA DISSOLUÇÃO  --------------------------------- VERIFICA DISSOLUÇÃO ----------------------- VERIFICA DISSOLUÇÃO
switch($_POST['dissolucao']){
...
//$data_fundacao=banco2data($data_fundacao);


// MANDATOS  ----------------------------- ELEIÇÃO DOS MEMBROS ---------------------------- ELEIÇÃO DOS MEMBROS


$m_presidente = $_POST['m_presidente'];
switch($m_presidente){
...
// MANDATO DIRETORIA  ---------------------------- MANDATO DIRETORIA -------------------------------- MANDATO DIRETORIA
$m_diretoria = $_POST['m_diretoria'];
$m_diretoria = "<span style='color:red !important;'>".$m_diretoria." anos</span>";

//$m_diretoria = "5 anos";

// ESCOLHA DIRETORIA  ---------------------------- ESCOLHA DIRETORIA -------------------------------- ESCOLHA DIRETORIA
switch($_POST['escolha_diretoria']){
...
// MOVIMENTAÇÕES FINANCEIRAS ------------------  MOVIMENTAÇÕES FINANCEIRAS -----------------------  MOVIMENTAÇÕES FINANCEIRAS
switch($_POST['m_financeiras']){
...
// ASSEMBLEIA GERAL - QUÓRUM  ------------------  ASSEMBLEIA GERAL - QUÓRUM -------------- ASSEMBLEIA GERAL - QUÓRUM
$ass_geral1 = "<span style='color:red !important;'>".$_POST['ass_geral1']."</span>";
$ass_geral2 = "<span style='color:red !important;'>".$_POST['ass_geral2']."</span>";

// CARGOS MINISTERIAIS ------------------------- CARGOS MINISTERIAIS ------------------------- CARGOS MINISTERIAIS
if(isset($_POST['cargom'])){
        $cargom = $_POST['cargom'];
...
// ORDENAÇÕES MINISTERIAIS ------------------------- ORDENAÇÕES MINISTERIAIS ------------------------- ORDENAÇÕES MINISTERIAIS
...
/*
$diretoria = $_POST['diretoria']  = ( isset($_POST['diretoria']) )  ? true : null;
$conselho = $_POST['conselho']  = ( isset($_POST['conselho']) )  ? true : "";
xxxxxxxxxxxxxssembleia = $_POST['assembleia']  = ( isset($_POST['assembleia']) )  ? true : null;
$ministerio =  $_POST['ministerio'] = ( isset($_POST['ministerio']) ) ? true : null;
...
?>
	...
	switch($governo){
		...
		case "3" :
			$governo = "<span style='color:red !important;'>Presbiterial</span>";
			$supensoes = "
			<p align=justify style='color:red !important;' ><strong>Art. $a".'º'." -</strong> Das suspensões:</br> </br>
					<strong>§1º -</strong> Qualquer membro, inclusive da Diretoria, que ficar suspenso por tempo indeterminado por não ser considerado de justa causa ou falta grave, ficará sem direito de votar e ser votado, devendo ser transcrito em ata e aprovada pela maioria dos membros da Liderança presentes através de uma Assembleia Geral Extraordinária convocada para esse fim;</br>
					<strong>§2º -</strong> Vencendo a sua suspensão, o membro voltará a ter seus direitos, devendo ser transcrito em ata e aprovada pela maioria dos membros da Liderança presentes através de uma Assembleia Geral Extraordinária convocada para esse fim.</p> ";
...
switch($_POST['remuneracao']){
		...
		case "3":
			...
			break;
				...
				<p align=justify ><strong>Art. $a".'º'." -</strong> A IGREJA, para ser mantida de modo eficiente e de acordo com a providência e a vontade de Deus, terá uma Diretoria composta dos seguintes membros, a saber: Presidente, $diretoria_vice, $diretoria_tesouraria, $diretoria_secretaria.</br></br>

				<strong>Parágrafo único -</strong> $mandato_presidente O restante dos membros da diretoria terão o mandato de $m_diretoria, podendo ser reeleitos de forma isolada ou conjunta.</p>";
...
switch($_POST['m_presidente']){
		...
		case "100":
			$a++;
			$mandato_presidente2 = "<p align=justify ><strong>Art. $a".'º'." -</strong> O fundador da IGREJA, $nome_presidente, será Presidente da IGREJA de maneira vitalícia, enquanto o mesmo estiver cumprindo os preceitos das Sagradas Escrituras e a Sã Doutrina de Nosso Senhor Jesus Cristo.</br></br>
			...
			<strong>§2° - </strong>Em caso de saída do cargo do presidente vitalício, e o mesmo estiver impossibilitado de indicar algum membro da Liderança para a ocupação do cargo de presidente, o <span style='color:red !important;'>Vice Presidente / Tesoureiro</span> assume o cargo de maneira direta.</p>";
			 break;
...
switch($_POST['conselho']){
	...
	$cap++;$a++;$capR = numberIntegerToRoman($cap, true);
    $conselho1 = "
